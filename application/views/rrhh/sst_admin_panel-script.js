...
function downloadFullReportExcel() {
    ...
    my_form.action = `${window.location.origin}/sst/sst/downloadFullReportExcelRrhh/`;
    my_form.method = "post";
...
function downloadFullReportPdf() {
    window.location = `${window.location.origin}/sst/sst/downloadFullReportPdfRrhh`;
}
...
function getRowsData() {
    ...
    return jQuery.get({
        "url": `${window.location.origin}/sst/sst/getFullReportRowsRrhh/`,
        "dataType": "json"		
    })
    .then(r => r);
